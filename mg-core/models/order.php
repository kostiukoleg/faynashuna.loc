...
class Models_Order {
  ...
  public function createCustomFieldToAdmin($data) {
    ...
    foreach ($fields as $item) {
      ...
      $data = null;
      foreach ($rows as $row) {
        if($row['field'] == MG::translitIt(trim($item['name']))) {
          $data = $row;
        }
      }
      $editableField = '';
        switch ($item['type']) {
        case 'input':
          $editableField = '<input name="'.MG::translitIt(trim($item['name'])).'" class="order-edit-display customField" type="text" style="display:none1;" value="'.$data['value'].'">';
          break;
        case 'textarea':
          $editableField = '<textarea name="'.MG::translitIt(trim($item['name'])).'" class="order-edit-display customField" type="text" style="display:none1;">'.$data['value'].'</textarea>';
          break;
        case 'checkbox':
          $checbox = true;
          if(empty($data['value'])) {
          ...
          $editableField = '<div class="checkbox">
                            <input type="checkbox" id="custom-'.MG::translitIt(trim($item['name'])).'" name="'.MG::translitIt(trim($item['name'])).'" '.$checked.' class="customField">
                            <label for="custom-'.MG::translitIt(trim($item['name'])).'"></label>
                          </div>';
          break;
        case 'select':
        case 'radiobutton':
          $editableField = '<select name="'.MG::translitIt(trim($item['name'])).'" class="order-edit-display customField">
            <option></option>';
          foreach ($item['variants'] as $variant) {
            if(htmlspecialchars_decode($data['value']) == $variant) {
              $selected = 'selected="selected"';
            } else {
              $selected = '';
            }
            $editableField .= '<option value="'.htmlspecialchars($variant).'" '.$selected.'>'.htmlspecialchars($variant).'</option>';
          }
