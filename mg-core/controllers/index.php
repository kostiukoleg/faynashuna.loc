...
class Controllers_Index extends BaseController {
  ...
  function __construct() {
    ...
    $res = DB::query('SELECT DISTINCT p.id, p.sort FROM '.PREFIX.'product AS p LEFT JOIN '.PREFIX.'product_variant AS pv ON p.id = pv.product_id
      WHERE p.new = 1 and p.activity=1 ORDER BY '.$sort.' ASC LIMIT '.$maxCountNew);
    while ($row = DB::fetchAssoc($res)) {
      $ids[] = $row['id'];
    }
    $res = DB::query('SELECT DISTINCT p.id, p.sort FROM '.PREFIX.'product AS p LEFT JOIN '.PREFIX.'product_variant AS pv ON p.id = pv.product_id
      WHERE (p.old_price>0 || pv.old_price>0) and p.activity=1 ORDER BY '.$sort.' ASC LIMIT '.$maxCountSales);
    while ($row = DB::fetchAssoc($res)) {
      $ids[] = $row['id'];
    }
